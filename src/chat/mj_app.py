...
class Mammamia(App):
    ...
    # 퇴장 메시지를 로그에 추가
        exit_text = Text(f"{exit_message['sender']}님이 {exit_message['message']} (보낸 시간: {exit_message['time']})", style="#d16a6a", justify="right")
    ...
    def post_message_to_log(self, sender, message, received_time):
        ...
        #    self.send_exit_message()
        #else:
        text_con = Text(f"{sender} : {message} (받은 시간 : {received_time})", style="#1a73c0", justify="right") # 받는 채팅은 우측으로
