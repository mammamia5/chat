    ...
    def on_input_submitted(self, event: Input.Submitted): # producer
        ...
        # 영화 검색 명령어 처리: @영화검색 키워드
            ...
            if keyword:
                ...
                # 영화 검색 결과를 출력
                if movie_results:
                    log_widget.write(Text(f"'{keyword}'에 대한 영화 검색 결과:", style="bold green"))

                    for idx, result in enumerate(movie_results, start=1):
                        directors = ', '.join(director.get('peopleNm', '정보 없음') for director in result.get('directors', []))
                        companys = ', '.join(company for company in result.get('companys', []))
                        movie_text = Text(
                        f"{idx}. 제목: {result.get('movieNm', '제목 없음')} ({result.get('prdtYear', '정보 없음')})\n"
                        f"영화 코드: {result.get('movieCd', '정보 없음')}\n"
                        f"개봉일: {result.get('openDt', '정보 없음')}\n"
                        f"장르: {result.get('genreAlt', '정보 없음')}\n"
                        f"국가: {result.get('nationAlt', '정보 없음')}\n"
                        f"상영 타입: {result.get('typeNm', '정보 없음')}\n"
                        f"상태: {result.get('prdtStatNm', '정보 없음')}\n"
                        f"감독: {directors}\n"
                        f"제작사: {companys}\n",
                        style="bold yellow"
                    )
                        log_widget.write(movie_text)
                else:
